        ...
        private fun addFlutterDeps(
            ...
            val copyFlutterAssetsTaskProvider: TaskProvider<Copy> =
                project.tasks.register(
                    ...
                    @Suppress("DEPRECATION")
                    // Set file mode if the Copy task implementation exposes the property.
                    // Some Gradle/Kotlin combinations don't have 'fileMode' available which
                    // causes a compilation error. Use reflection to set it when present and
                    // otherwise silently continue.
                    try {
                        val fileModeField = this.javaClass.getDeclaredField("fileMode")
                        fileModeField.isAccessible = true
                        // 'fileMode' is an Int in the Flutter plugin sources.
                        fileModeField.setInt(this, 420)
                    } catch (_: NoSuchFieldException) {
                        // Older/newer Gradle versions might not expose fileMode; ignore.
                    } catch (_: Throwable) {
                        // Any other reflection issue should not break the build.
                    }
